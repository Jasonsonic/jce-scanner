...
def generate_dashboard(csv_path: Path, out_path: Path):
    ...
    doc = f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>JCE Scanner V3.4</title><style>
...
</style></head><body><header><h1>JCE Scanner V3.4</h1><div class="meta">最新交易日双策略扫描 · 更新：{generated_at} · 共{len(df)}只</div></header><section class="cards">{cards}</section><div class="actions"><a href="jce_scan_v3_4.xlsx">下载 Excel</a><a href="jce_scan_v3_4.csv">下载 CSV</a></div><div class="table-wrap"><table><thead><tr><th>代码</th><th>优先分</th><th>主信号</th><th>最终建议</th><th>JCE压缩分</th><th>连跌首阳分</th><th>下跌阶段跨度</th><th>实际下跌日</th><th>忽略的小涨日</th><th>累计跌幅</th><th>首阳涨幅</th><th>首阳量比</th><th>四线宽度</th><th>收盘距MA60</th><th>反转状态</th></tr></thead><tbody>{''.join(rows)}</tbody></table></div><footer>同时识别均线压缩贴近MA60与连续下跌后首根确认阳线。技术研究工具，不构成投资建议。</footer></body></html>'''
...
def main():
    ...
    csv_path = OUTPUT_DIR / "jce_scan_v3_4.csv"
    xlsx_path = OUTPUT_DIR / "jce_scan_v3_4.xlsx"
    generate_dashboard(csv_path, SITE_DIR / "index.html")
    (SITE_DIR / ".nojekyll").write_text("", encoding="utf-8")
    (SITE_DIR / "jce_scan_v3_4.csv").write_bytes(csv_path.read_bytes())
    (SITE_DIR / "jce_scan_v3_4.xlsx").write_bytes(xlsx_path.read_bytes())
    return 0
